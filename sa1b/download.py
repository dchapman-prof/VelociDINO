...
parser.add_argument('--n_img', type=int, default=-1, help='Number of random images to download')
...
# Do we only want some random images
if args.n_img>=0:
	...
	# Slice the images
	if args.n_img > len(lines):
		print('ERROR: cannot pick %d images, there are only %d images to pick from' % args.n_img, len(lines))
		sys.exit(1)
	lines = lines[0:args.n_img]
